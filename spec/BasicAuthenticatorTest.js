...
import BasicAuthenticator from "../lib/BasicAuthenticator.js";
...
describe("Login URL", function () {
  ...
  it("should have a default target", function () {
    ...
    var expTarget =
      "?target=" +
      encodeURIComponent("https://" + request.headers.host + request.url);
    expect(auth.buildLoginURL()).to.equal(expected + expTarget);
  });

  it("should contain a URL encoded target URL", function () {
    var auth = new BasicAuthenticator(request);
    var loginURL = auth.buildLoginURL({
      target: "https://example.com/targetURL",
    });
    expect(loginURL).to.include(
      "target=" + encodeURIComponent("https://example.com/targetURL")
    );
  });
  ...
  it("should use the MKey authnContext", function () {
    var auth = new BasicAuthenticator(request);
    expect(auth.buildLoginURL({ mkey: true })).to.include(
      "authnContextClassRef=" + encodeURIComponent(auth.UMN_MKEY_AUTHN_CONTEXT)
    );
  });

  it("should use the Duo authnContext", function () {
    var auth = new BasicAuthenticator(request);
    expect(auth.buildLoginURL({ duo: true })).to.include(
      "authnContextClassRef=" + encodeURIComponent(auth.UMN_DUO_AUTHN_CONTEXT)
    );
  });
});
...
describe("Logout URL", function () {
  ...
  it("should begin with the SP logout endpoint", function () {
    var auth = new BasicAuthenticator(request);
    expect(auth.buildLogoutURL()).to.match(
  ...
  it("should contain a double-encoded return URL returning from IdP logout", function () {
    var auth = new BasicAuthenticator(
      ...
      response,
      {},
      { return: "https://example.com/returnURL?param=123" }
    );
    expect(auth.buildLogoutURL()).to.include(
      "return%3D" + encodeURIComponent(encodeURIComponent(returl))
    );
  });

  it("should not logout from the IdP", function () {
    var auth = new BasicAuthenticator(request);
    expect(auth.buildLogoutURL({ logoutFromIdP: false })).to.not.include(
      "return=" + encodeURIComponent(auth.UMN_IDP_LOGOUT_URL)
    );

    var logoutNoIdPButReturn = auth.buildLogoutURL({
      logoutFromIdP: false,
      return: returl,
    });
    expect(logoutNoIdPButReturn).to.not.include(
      "return=" + encodeURIComponent(auth.UMN_IDP_LOGOUT_URL)
    );
  ...
  it("should accept an alternate logout endpoint", function () {
    var auth = new BasicAuthenticator(request);
    expect(
      auth.buildLogoutURL({ LogoutFromIdP: true, IdPLogoutURL: altLogout })
    ).to.include("return=" + encodeURIComponent(altLogout));
  });

  it("should accept an alternate logout endpoint with a double-encoded final return URL", function () {
    var auth = new BasicAuthenticator(request);
    var logoutIdPAltWithReturn = auth.buildLogoutURL({
      logoutFromIdP: true,
      IdPLogoutURL: altLogout,
      return: returl,
    });
    expect(logoutIdPAltWithReturn).to.include(
      "return=" + encodeURIComponent(altLogout)
    );
    expect(logoutIdPAltWithReturn).to.include(
      "return%3D" + encodeURIComponent(encodeURIComponent(returl))
    );
...
describe("Login/Logout Redirects", function () {
  ...
  it("should redirect to logout with expected parameters", function () {
    ...
    expect(response.getHeader("Location")).to.include(
      "return=" + encodeURIComponent(auth.UMN_IDP_LOGOUT_URL)
    );
  });
});
...
describe("Session attributes", function () {
  ...
  it("should return multivalue attributes as an array or null", function () {
    ...
    var auth = new BasicAuthenticator(request);
    expect(auth.getAttributeValues("umnLibAccess")).to.have.all.members([
      "1",
      "2",
      "9",
    ]);
    expect(auth.getAttributeValues("customDelimiter", ",")).to.have.all.members(
      ["one", "two", "three"]
    );
  ...
  it("should return an object containing multiple requested attributes", function () {
    var request = createRequest({
      ...
      headers: {
        "shib-identity-provider": "https://idp2.shib.umn.edu/idp/shibboleth",
        "shib-authentication-instant": new Date(
          (new Date().getTime() / 1000 - 20) * 1000
        ).toISOString(),
        host: "example.com",
        umnLibAccess: "1;2;9",
        customDelimiter: "one,two,three",
  ...
  it("should be logged in with MKey", function () {
    var mkeyRequest = createRequest({
      url: "/",
      headers: {
        host: "example.com",
        "shib-identity-provider": "https://idp2.shib.umn.edu/idp/shibboleth",
        "shib-authentication-method": new BasicAuthenticator()
          .UMN_MKEY_AUTHN_CONTEXT,
      },
    });
  ...
  it("should be logged in with Duo", function () {
    var duoRequest = createRequest({
      url: "/",
      headers: {
        host: "example.com",
        "shib-identity-provider": "https://idp2.shib.umn.edu/idp/shibboleth",
        "shib-authentication-method": new BasicAuthenticator()
          .UMN_DUO_AUTHN_CONTEXT,
      },
    });
  ...
  it("should handle timeouts based on specified maxAge", function () {
    ...
    aRequest.headers["shib-authentication-instant"] = new Date(
      (new Date().getTime() / 1000 - 20) * 1000
    ).toISOString();
    var auth = new BasicAuthenticator(aRequest);
  ...
  it("should report the correct IdP", function () {
    var auth = new BasicAuthenticator(request);
    expect(auth.getIdpEntityId()).to.equal(
...
describe("Attribute access", function () {
  it("should default to HTTP header access", function () {
    var auth = new BasicAuthenticator();
    expect(auth.getAttributeAccessMethod()).to.equal(
      new BasicAuthenticator().UMN_ATTRS_FROM_HEADERS
    );
  });
  ...
  it("should accept known values for access methods", function () {
    ...
    expect(auth.getAttributeAccessMethod()).to.equal(
      new BasicAuthenticator().UMN_ATTRS_FROM_ENV
    );
    auth.setAttributeAccessMethod(auth.UMN_ATTRS_FROM_HEADERS);
    expect(auth.getAttributeAccessMethod()).to.equal(
      new BasicAuthenticator().UMN_ATTRS_FROM_HEADERS
  ...
  it("should not accept an unknown access method value", function () {
    var auth = new BasicAuthenticator();
    expect(function () {
      auth.setAttributeAccessMethod("badvalue");
    }).to.throw("Invalid attribute access method");
  });

  it("should return the known default set of common attributes", function () {
    var auth = new BasicAuthenticator();
    expect(auth.getDefaultAttributeNames()).to.have.all.members([
      "uid",
      "eppn",
      "isGuest",
      "umnDID",
  ...
  it("should return the expected set of merged attribute names", function () {
    var auth = new BasicAuthenticator();
    expect(
      auth.getAttributeNames(["attr1", "attr2", "attr3"])
    ).to.have.all.members([
      "uid",
      "eppn",
      "isGuest",
  ...
  it("should throw an error if a non-array was passed in", function () {
    var auth = new BasicAuthenticator();
    expect(function () {
      auth.getAttributeNames("not an array");
    }).to.throw("requestedAttributes must be an Array");
  });
});
...
describe("Attribute normalization", function () {
  it("should convert a HTTP header to a properly cased attribute name", function () {
    var auth = new BasicAuthenticator();
    auth.setAttributeAccessMethod(auth.UMN_ATTRS_FROM_HEADERS);
    expect(auth.normalizeAttributeName("HTTP_AN_ATTRIBUTE")).to.equal(
      "an-attribute"
    );

    auth.setAttributeAccessMethod(auth.UMN_ATTRS_FROM_ENV);
    expect(auth.normalizeAttributeName("AN_ATTRIBUTE")).to.equal(
      "AN_ATTRIBUTE"
    );
  });
});
