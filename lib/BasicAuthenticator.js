...
function BasicAuthenticator(loginOptions, logoutOptions, request) {
  this.loginOptions = loginOptions || {};
  this.logoutOptions = logoutOptions || {};

  var _handlerURL = '/Shibboleth.sso';
  var _attributeSource = this.UMN_ATTRS_FROM_HEADERS;
...
BasicAuthenticator.prototype.buildLoginURL = function(options) {
  ...
  if (options.target) {
    ...
    loginURL += encodeURIComponent(this._request.hostname + this._request.uri);
  }
  loginURL
  return 'https://' + request.hostname + request.uri + '?target=';
};
